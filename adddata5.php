		...
		if($role=="Admin")
		{
			echo"<script>window.location.assign('admdata.php');</script>";
		}
		// Include config file
...
if(isset($_POST['save']))
{
	...
	if(($extno[0] !== '2' or $extno[0] !== '3' or $extno[0] !== '3' or $extno[0] !== 'A') and strlen($extno) > 4) 
	 ...
	 if ($user) { // if user exists
		 ...
		 $blockname=mysqli_real_escape_string($conn,$_POST["block"]);
		 }
	...
	//echo "<script>alert('$d');</script>";
		...
		if(empty($uname_err) && empty($Ext_err) && empty($type_err) && empty($berr) && empty($lnerr) && empty($baerr) && empty($cn2err) && empty($cc2err) && empty($saerr) && empty($eaerr) && empty($ecnerr) && empty($eccerr) && empty($colorcode_err) && empty($type_err) && empty($Ext_err) && empty($cable_err))
		{
...
function r1()
{
	...
	<?php } ?>	
		...
		{ ?>
                                                ...
                                                <span class="help-block" style="color:red;font:6px" id="enerr" name="cerr"><?php echo $uname_err; ?></span>
																						...
																						<br>			<label for="example_input_full_name"  style="font-weight: bold; font-family: helvetica;">
																									...
																									<input type="text" name="color[]" id="col" class="form-control m-input" style="width: 200px; margin-left: 181px; margin-top: -33px;" value="" placeholder="">
												...
												<div class="form-group m-form__group  <?php echo (!empty($uname_err)) ? 'has-error' : ''; ?>" id="sub"  style="margin-top: 25px;">
																								<fieldset style= "border :TRUE">
		...
		{ ?>
														<textarea  placeholder="Comments Here" style="size: 100%; margin-left:180px; height: 80px; width: 210px; margin-top: -38px"  name="cmt"  id="cmt" type="text" class="txt_3" value="<?php echo $_SESSION['acm'] ?>"><?php echo $_SESSION['ascc'] ?></textarea>
														<?php } else { ?>	
															<textarea  placeholder="Comments Here" style="size: 100%; margin-left:180px; height: 80px; width: 210px; margin-top: -38px"  name="cmt"  id="cmt" type="text" class="txt_3"></textarea>
															<?php } ?>
													</div>
											</div>
										</div>
