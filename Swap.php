				...
				$Ext_err = "Please Enter valid Extension number";
					...
					if ($user && $user1)  { // if user exists
						if (($user['Ext_No'] === $e1) &&($user1['Ext_No'] === $e2)) {
							...
							$hop_check_query1 = "SELECT * FROM hops WHERE Ext_No='$e2'";
					...
					$e2c=count($huser1);
					if($huser && $huser1)  { 
						//echo"<script>alert('$e1c $e2c');</script>";
						// if user exists
						if (($e1c>0) && ($e2c>0)) {
					...
					if(($e1c>0) && ($e2c<=0))  {
						...
						$h1="update hops set Ext_No='$e2' where Ext_No='$e1'";
							mysqli_query($conn,$h1);
					} 
					if(($e1c<=0) && ($e2c>0))  {
...
-->
		...
		<!--begin::Portlet-->
							...
							<form method="post">
										<button type="submit" class="btn btn-primary"style="postion:fixed; margin-left:500px; margin-top:10px" value="Submit">Swap</button>
</form>		<!--end::Form-->
</form>
